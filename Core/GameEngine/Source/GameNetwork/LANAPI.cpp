...
void LANAPI::init()
{
	...
	m_transport->enableVirtualAddressing(lobbyPort);
...
//	myGame->setInProgress(false);
	...
	newSlot.setPort((UnsignedShort)(LAN_INGAME_PORT_BASE + rts::ClientInstance::getInstanceIndex()));
...
Bool LANAPI::SetLocalIP( UnsignedInt localIP )
{
	Bool retval = TRUE;
	const UnsignedShort instanceOffset = (UnsignedShort)rts::ClientInstance::getInstanceIndex();
	m_localIP = Transport::makeVirtualIP(localIP, instanceOffset);
